...
<head>
	...
	<!-- 탭 명 -->
	<title>박스오피스 | 시간대 유형 관리</title>
	<style type="text/css">
		#wrap_admin_time_manage {
			width: 1000px;
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$.ajax({
				url: "/TicketingProject/admin/time.do"
				, type: "post"
// 				, data: ""
				, dataType: "json"
...
// 				,"padding-bottom": "200px"
		...
		function removeRows() {
			...
			if(checkedRows.length == 0) {
				...
				checkedRows.each(function(idx, item) {
					...
					$(item).parent().parent().children(".time_cd").append("삭제예정");
					$(item).parent().parent().children(".time_cdDesc").append("삭제예정");
		...
		function changeValue(obj) {
			...
			var str = "<input id='insertTimeValue' type='text' value='" + val + "' onfocusout='saveValue(this)'>"
			$(obj).append(str);
			$(obj).append(ele);
			$("#insertTimeValue").focus();
		}
		
		function saveValue(obj) {
			var val = $(obj).val();
			
			if($(obj).parent().attr("class") == "time_cd") {
				$(obj).parent().children("[name='cdName']").val(val);
			} else if($(obj).parent().attr("class") == "time_cdDesc") {
				$(obj).parent().children("[name='cdDesc']").val(val);
			}
			
			$(obj).parent().append(val);
			console.log($(obj).parent().parent().children("[name='flag']").val("M"));
			$("#insertTimeValue").remove();
			$("#table_changed_check").val("changed");
		}
		...
		function saveTable() {
			...
			var eleTd = $("#table_time tbody td.time_cd");
			...
			eleTd = $("#table_time tbody td.time_cdDesc");
			for(var i=0 ; i<eleTd.length ; i++) {
				if($(eleTd[i]).text() == null || $(eleTd[i]).text() == "") {
					alert("값이 입력되지 않은 항목이 있습니다.");
					return;
			...
			$("#table_time tbody tr").each(function(idx, item) {
				var flag = $(item).children("[name='flag']").val();
				var cdStr = $(item).children(".time_cd").children("[name='cd']").val();
				var cdNameStr = $(item).children(".time_cd").children("[name='cdName']").val();
				var cdDescStr = $(item).children(".time_cdDesc").children("[name='cdDesc']").val();
				var deleteYnStr = $(item).children(".time_deleteYn").children("[name='deleteYn']").val();
				var url = "";
				...
				if(flag == null || flag == "") {
					...
					if(cdStr == null || cdStr == "") {
						url = "/TicketingProject/admin/timeCreate.do";
					} else {
						console.log(1);
						url = "/TicketingProject/admin/timeModify.do";
					}
				} else if(flag == "D") {
					url = "/TicketingProject/admin/timeDelete.do";
				}
...
<body>
	...
	<div id="wrap_admin_time_manage">
		<div class="container" style="margin-top: 100px">
			<h2 class="title">시간대 관리<span>시간대 유형 관리</span></h2>
			<hr>
			<form>
				<table id="table_time" class="table table-hover table-striped table_time">
					<thead>
						<tr>
							<th>선택</th>
							<th>시간대 유형</th>
