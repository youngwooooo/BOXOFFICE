...
<head>
	...
	<!-- 탭 명 -->
	<title>박스오피스 | 관람객 유형 관리</title>
	<style type="text/css">
		#wrap_admin_audience_manage {
			width: 1000px;
...
/* 			margin-top:	50px; */
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$.ajax({
				url: "/TicketingProject/admin/audience.do"
				, type: "post"
// 				, data: ""
				, dataType: "json"
		...
		function makeTable(data) {
			...
			$("#table_audience tbody").append(str);
		}
		...
		function removeRows() {
			...
			if(checkedRows.length == 0) {
				...
				checkedRows.each(function(idx, item) {
					...
					$(item).parent().parent().children(".audience_cd").append("삭제예정");
					$(item).parent().parent().children(".audience_cdDesc").append("삭제예정");
		...
		function changeValue(obj) {
			...
			var str = "<input id='insertAudienceValue' type='text' value='" + val + "' onfocusout='saveValue(this)'>"
			$(obj).append(str);
			$(obj).append(ele);
			$("#insertAudienceValue").focus();
		}
		
		function saveValue(obj) {
			var val = $(obj).val();
			
			if($(obj).parent().attr("class") == "audience_cd") {
				$(obj).parent().children("[name='cdName']").val(val);
			} else if($(obj).parent().attr("class") == "audience_cdDesc") {
				$(obj).parent().children("[name='cdDesc']").val(val);
			}
			
			$(obj).parent().append(val);
			console.log($(obj).parent().parent().children("[name='flag']").val("M"));
			$("#insertAudienceValue").remove();
			$("#table_changed_check").val("changed");
		}
		...
		function saveTable() {
			...
			var eleTd = $("#table_audience tbody td.audience_cd");
			...
			eleTd = $("#table_audience tbody td.audience_cdDesc");
			for(var i=0 ; i<eleTd.length ; i++) {
				if($(eleTd[i]).text() == null || $(eleTd[i]).text() == "") {
					alert("값이 입력되지 않은 항목이 있습니다.");
					return;
			...
			$("#table_audience tbody tr").each(function(idx, item) {
				var flag = $(item).children("[name='flag']").val();
				var cdStr = $(item).children(".audience_cd").children("[name='cd']").val();
				var cdNameStr = $(item).children(".audience_cd").children("[name='cdName']").val();
				var cdDescStr = $(item).children(".audience_cdDesc").children("[name='cdDesc']").val();
				var deleteYnStr = $(item).children(".audience_deleteYn").children("[name='deleteYn']").val();
				var url = "";
				...
				if(flag == null || flag == "") {
					...
					if(cdStr == null || cdStr == "") {
						url = "/TicketingProject/admin/audienceCreate.do";
					} else {
						console.log(1);
						url = "/TicketingProject/admin/audienceModify.do";
					}
				} else if(flag == "D") {
					url = "/TicketingProject/admin/audienceDelete.do";
				}
...
<body>
	...
	<div id="wrap_admin_audience_manage">
		<div class="container" style="margin-top: 100px">
			<h2 class="title">관람객 관리<span>관람객 유형 관리</span></h2>
			<hr>
			<form>
				<table id="table_audience" class="table table-hover table-striped table_audience">
					<thead>
						<tr>
							<th>선택</th>
							<th>관람객 유형</th>
