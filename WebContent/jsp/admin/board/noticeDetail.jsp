 ...
 */
	...
	function callHandler(button) {
		...
		if(button.id == "updateNT") {
			window.location.href = 'updateNotice.do?postCd=<%=postVO.getPostCd()%>';
			
		}else if(button.id == "deleteNT") {
			if(!confirm("공지사항을 삭제하시겠습니까?")) {
				return;
			}else {
				window.location.href = 'deleteNotice.do?postCd=<%=postVO.getPostCd()%>';			
			}
